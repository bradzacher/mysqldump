...
function parseGeometryValue(buffer: Buffer): string {
  ...
  // eslint-disable-next-line complexity -- complexity is sectioned off in cases
...
function typeCast(tables: Array<Table>): (field: TypecastField) => string {
  ...
  // eslint-disable-next-line complexity -- complexity is split into ifs
  return (field: TypecastField) => {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- shouldn't be null, ever
    ...
    /* istanbul ignore else */ if (columnType === 'GEOMETRY') {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- types are incorrect
      if (buf == null) {
        value = null;
      } else {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- need this to allow unhandled case handling
    } else if (columnType === 'NUMBER') {
      value = field.string();
    } else {
      // eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- unhandled case handling
      throw new Error(`Unknown column type detected: ${columnType as string}`);
    }
