...
async function getTriggerDump(
  ...
  const triggers = (
    await connection.query<ShowTriggers>(`SHOW TRIGGERS FROM \`${dbName}\``)
  )
  ...
  result
    .map(r => r[0])
    .forEach(res => {
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion -- should always exist
      ...
      if (options.delimiter !== false && options.delimiter.length > 0) {
