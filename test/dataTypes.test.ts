/* eslint-disable @typescript-eslint/no-non-null-assertion -- tests */
...
describe('insert data types', () => {
  ...
  function numberTypesTest(matches: string): void {
    ...
    let i = 0;
    /* eslint-disable no-plusplus -- easy way of defining this block */
    ...
    expect(values[i++]).toMatch(/^NULL$/);
    expect(values[i++]).toMatch(/^NULL$/);
    /* eslint-enable no-plusplus -- easy way of defining this block */
  }
  it(
